...
def run_decode(background=False):
    ...
    gen_tokens = []
    t0 = time.perf_counter()
    for i, (token, _) in enumerate(generate_step(prompt_arr, model, max_tokens=MAX_TOKENS)):
        if i == 0:
...
def main():
    ...
    parser.add_argument("--background", action="store_true", help="バックグラウンドロード有効")
    ...
    if args.background:
        ...
        avg_expert_ms = (
            result["bg_load_total_dt"] / result["bg_load_count"] * 1000 if result["bg_load_count"] > 0 else 0
        )
